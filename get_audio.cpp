...
string char_to_string(const char * chars){
    return string(chars);
}

char* string_to_char(string str){
    //先分配空间，new char[]是c++中的东西
 ...
 * 从多媒体文件中抽取媒体信息
 * */
string get_audio(string input_filename){
    ...
    const char *  intput_f = string_to_char(input_filename);
    const char *  output_f = "test.aac";


    // 1. 读取多媒体文件
    int ret = avformat_open_input(&fmt_ctx, intput_f, NULL, NULL);
    if (ret < 0) {
    ...
    // return "./build/" + output_filename;
    return "./build/"  + char_to_string(output_f);
...
int main(int argc, char *argv[]) {
    //输入和输出文件名称
    string input_filename = "/Users/zack/Desktop/test.mp4";
    string output_filename =  get_audio(input_filename);
